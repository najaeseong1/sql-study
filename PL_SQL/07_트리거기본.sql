...
BEGIN
    ...
    -- :OLD.id 원본 테이블에서 수정전, 삭제 될 값
    VALUES(:OLD.id,:OLD.name,:OLD.address,sysdate,v_type,USER()); -- USER() == 현재 로그인 중인 유저 값
END;
...
CREATE OR REPLACE TRIGGER trg_user_insert
    ...
    FOR EACH ROW
BEGIN
    :NEW.name := SUBSTR(:NEW.name, 1, 1) || RPAD('*', LENGTH(:NEW.name) - 1, '*');
END;
...
DECLARE
    ...
    quantity_shortag_exception EXCEPTION;
    zero_total_exception EXCEPTION;
BEGIN
    dbms_output.put_line('BEFORE INSERT 트리거 실행');
    v_total := :NEW.total;
    v_product_no := :NEW.product_no;
    
    SELECT total INTO v_product_total FROM product WHERE product_no = v_product_no;
    SELECT :NEW.total INTO v_total FROM dual;
    
    IF v_product_total <= 0
    THEN  
        RAISE zero_total_exception;
    ELSIF v_total>v_product_total
    THEN
        RAISE quantity_shortag_exception;
    END IF;    
    UPDATE product SET total = total - v_total
    WHERE product_no = v_product_no;
    
    EXCEPTION 
        WHEN quantity_shortag_exception THEN
            -- 오라클에서 제공하는 사용자 정의 예외를 발생시키는 함수
            -- 첫번째 매개값: 에러 코드 (사용자 정의 예외 코드는 -20000~20999까지)
            -- 두번째 매개값: 에러 메세지
            RAISE_APPLICATION_ERROR(-20001, '주문하신 수량보다 재고가 적어서 주문할 수 없습니다.');
        WHEN zero_total_exception THEN
            RAISE_APPLICATION_ERROR(-20002, '주문한 상품의 재고가 없어 주문할 수 없습니다.');
END;
...
INSERT INTO order_history VALUES(order_history_seq.NEXTVAL, 200,3,5,30000);
