...
WHERE 
    e.department_id = d.department_id; -- 오라클 조인 문법
...
/*
1. loc 태이블의 state_province = 'California' 조건에 맞는 값을 대상으로
...
어느 한 테이블에 공통 값이 없더라도 해당 row들이 조회 결과에 모두 포함되는
조인을 말합니다.
*/
...
SELECT
    ...
    loc.location_id
FROM 
    employees e, departments d, locations loc
WHERE 
    e.department_id = d.department_id(+)
...
(+) 태그가 붙지 않은 테이블을 기준으로 하여 departments 테이블이 
조인에 참여하라는 의미를 부여하기위해 기호를 붙입니다
