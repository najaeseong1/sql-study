...
SELECT TO_CHAR(sysdate, 'YY-MM-DD DY HH24:MI:SS')FROM dual; -- HH24는 24시간으로 표기
...
-- 형식에서 사용하는 '9'가 실제 숫자 9가 아니라 자릿수를 표현하기 위한 기호입니다.
SELECT TO_CHAR(20000,'99999')FROM dual;
-- 주어진 자릿수에 숫자를 모두 표기할 수 없어서 모두 #으로 표기됩니다.
SELECT TO_CHAR(20000,'9999')FROM dual;
-- 소수점 자릿수가 부족하면 절삭이 아닌 반올림 실행
SELECT TO_CHAR(20000.29,'99999.9999')FROM dual;
SELECT TO_CHAR(20000,'$0,009,999')FROM dual;
...
SELECT TO_DATE('2020/12/25 12:23:50','YY-MM-DD HH:MI:SS') FROM dual;
...
SELECT
    first_name,
    commission_pct,
    salary+commission_pct, -- null은 연산이 되지 않습니다
    nvl2(
        commission_pct,
...
FROM
    employees
WHERE
    TRUNC((sysdate-hire_date)/365,0) >= 17
ORDER BY 근속년수 DESC;    
/*
문제 2.
